...
public class CuckooHash<K, V> {
    ...
    public void put(K key, V value) {
        ...
        Bucket<K, V> firstBucket = new Bucket<>(key, value);
        int nextHash = hash1(firstBucket.bucKey);
        Bucket<K, V> secondBucket = table[nextHash];

        // Place the bucket in the hash table, with the CAPACITY displacements
        for (int i = 0; i < CAPACITY; i++) {
            // Check if there is an identical pair that is already here
            if (secondBucket != null && secondBucket.getBucKey().equals(firstBucket.bucKey) && secondBucket.getValue().equals(firstBucket.getValue())) {
                return; // If the key-value pair already exists, do nothing
            }

            // Place the new bucket in the current position
            table[nextHash] = firstBucket;

            // If the current position is empty, it was inserted, so return
            if (secondBucket == null) {
                return;
            }

            //Move the bucket that is currently here
            nextHash = nextHash == hash1(secondBucket.getBucKey()) ? hash2(secondBucket.getBucKey()) : hash1(secondBucket.getBucKey());
            firstBucket = secondBucket;
            secondBucket = table[nextHash];
        }

        rehash();

        // After rehashing, try to reinsert the displaced element
        put(firstBucket.getBucKey(), firstBucket.getValue());
    }
